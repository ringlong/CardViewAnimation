...
- (void)panHandle:(UIPanGestureRecognizer *)pan {
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        [self setCenter:CGPointMake(CGRectGetWidth(self.view.bounds) / 2,
                                    CGRectGetHeight(self.view.bounds) / 2 + 150 - 20 - (sizePercent * 0 * 400))
               Duration:0.25
                   Card:cardView
                  Index:0];
    ...
    card.center = CGPointMake(card.center.x + transLcation.x,
                              card.center.y + transLcation.y);
    ...
    for (int i = 1; i < self.cards.count - 1; i++) {
        ...
        CGPoint subCardCenter = CGPointMake(CGRectGetWidth(self.view.bounds) / 2,
                                            CGRectGetHeight(self.view.bounds) / 2 + 150 - 20 - sizePercent * 400 * (i - 1 + (fabs(percent))));
        NSLog(@"%@",@(XOffPercent));
...
- (void)showCardZone {
    for (int i = 0; i < 4; i++) {
        ...
        CGPoint targetCenter = CGPointMake(CGRectGetWidth(self.view.bounds) / 2,
                                           CGRectGetHeight(self.view.bounds) / 2 + 150 - 20 - (sizePercent * i * 400));
        [self setCenter:targetCenter Duration:0.1f Card:card Index:i];
        
        card.center = CGPointMake(CGRectGetWidth(self.view.bounds) / 2,
                                  CGRectGetHeight(self.view.bounds) / 2 - 10 * i + 50);
...
- (void)cardReCenterOrDismiss:(BOOL)isDismiss Card:(CardView *)card{
    if (isDismiss) {
        ...
        [self setRorationWithAngle:0 Duration:0.25 Card:card];
        CGPoint targetCenter = CGPointMake(CGRectGetWidth(self.view.bounds) / 2,
                                           CGRectGetHeight(self.view.bounds) / 2 + 150 - 20 - (sizePercent * 3 * 400));
