...
- (void)config{
    ...
    maskLayer.path =
    [UIBezierPath bezierPathWithRoundedRect:self.layer.bounds
                          byRoundingCorners:UIRectCornerAllCorners
                                cornerRadii:CGSizeMake(self.cornerRadius,
    ...
    self.circleView2.center = CGPointMake(CGRectGetWidth(self.bounds) / 2,
                                          CGRectGetHeight(self.bounds) / 2);
    
    self.circleView1.transform = CGAffineTransformScale(self.circleView1.transform, 0.5, 0.5);
    self.circleView1.center = CGPointMake(15 + self.circleSize.width / 2 / 2,
                                          15 + self.circleSize.width / 2 / 2);
...
- (IBAction)tapClick:(id)sender {
    BOOL firstIsCenter;
    if (CGPointEqualToPoint(CGPointMake(CGRectGetWidth(self.bounds) / 2,
                                        CGRectGetHeight(self.bounds) / 2),
                            self.circleView1.center)) {
        firstIsCenter = YES;
    } else {
        firstIsCenter = NO;
    }
    ...
    if (firstIsCenter) {
        ...
        bAni1.toValue = [NSValue valueWithCGPoint:CGPointMake(15 + self.circleSize.width / 2 / 2,
                                                              15 + self.circleSize.width / 2 / 2)];
        bAni2.toValue = [NSValue valueWithCGPoint:CGPointMake(CGRectGetWidth(self.bounds) / 2,
                                                              CGRectGetHeight(self.bounds) / 2)];
    } else {
        sSani1.toValue = [NSValue valueWithCGPoint:CGPointMake(1, 1)];
        sSani2.toValue = [NSValue valueWithCGPoint:CGPointMake(0.5, 0.5)];
        
        bAni2.toValue = [NSValue valueWithCGPoint:
                         CGPointMake(CGRectGetWidth(self.bounds) - 15 - self.circleSize.width / 2 / 2,
                                     15 + self.circleSize.width / 2 / 2)];
        bAni1.toValue = [NSValue valueWithCGPoint:CGPointMake(CGRectGetWidth(self.bounds) / 2,
                                                              CGRectGetHeight(self.bounds) / 2)];
    }
    ...
    [butBani setCompletionBlock:^(POPAnimation *ani, BOOL isFinished) {
        if (isFinished) {
            ...
            butBani1.toValue = [NSValue valueWithCGSize:
                                CGSizeMake(CGRectGetWidth(self.bounds) * 0.8, 40)];
